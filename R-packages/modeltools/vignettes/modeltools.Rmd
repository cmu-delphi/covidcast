...
Delphi's `modeltools` package can be installed from our 
[**GitHub page**](https://github.com/cmu-delphi/covidcast/tree/main/R-packages/modeltools)
with the command below. The CRAN `modeltools` package is entirely unrelated to 
this one. 

```{r, eval = F}
devtools::install_github(repo = "cmu-delphi/covidcast", subdir = "R-packages/modeltools",
                         ref = "collin-develop", build_vignettes = TRUE)
```


## Downloading COVIDcast signals

In these vignettes, we'll illustrate the suite of tools provided by the 
`modeltools` package for analyzing, manipulating, and forecasting the COVID-19 
signals available in Delphi's
[**COVIDcast Epidata API**](https://cmu-delphi.github.io/delphi-epidata/api/covidcast_signals.html).
The examples we provide here will center around the progression of COVID-19 case 
rates (i.e. the number of new COVID-19 cases reported per day, per 100,000 
population), as reported by [**USAFacts**](https://usafacts.org/), over the 
course of the pandemic. The API documentation for USAFacts signals can be found
[**here**](https://cmu-delphi.github.io/delphi-epidata/api/covidcast-signals/usa-facts.html).


The code chunk below utilizes Delphi's COVIDcast API <tt>R</tt> client to fetch
the reported case rates for four U.S. states --- CA, FL, NY, and PA --- starting
on March 1, 2020. See the `covidcast` 
[**Getting started guide**](https://cmu-delphi.github.io/covidcast/covidcastR/articles/covidcast.html)
for more examples on how to use the Delphi API's <tt>R</tt> client.
...
As with other signals stored in Delphi's API, the API also computes and stores 
a smoothed version of the USAFacts case rates signal --- a 7-day trailing 
average of the (raw) daily case rates reported by USAFacts. The code chunk above
pulls both the raw and smoothed signals from the API and aggregates them
into the "wide format" data frame displayed below.
...
col3.def <- colDef(minWidth = 175, html = T, format = colFormat(digits = 3), 
...
bscols(case_rates_tbl, list(filter_select("geo_value", "State", case_rates_data, ~toupper(geo_value), multiple = F)),
...
The reported USAFacts case rates, with 7-day trailing averages superposed, are
plotted below for each of the four states.
