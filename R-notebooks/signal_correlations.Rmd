...
par(mfrow=c(1, N_county-1), mar=c(4.5, 4.5, 5.5, 0.5))
...
for (i in inds_county[-length(inds_county)]) {
  y = signal_dfs[[i]] %>% 
    filter(is_county) %>%
    group_by(geo_value) %>%
    summarize(value = mean(value, na.rm=TRUE))
    
  main = paste("jhu-cases-ahead", "vs", sources[i])
  cor_plot(x, y, county_pop, main=main)
}

# Average over available weeks per MSA
par(mfrow=c(1, N_msa-1), mar=c(4.5, 4.5, 5.5, 0.5))
...
for (i in inds_msa[-length(inds_msa)]) {
  y = signal_dfs[[i]] %>% 
    filter(!is_county) %>%
    group_by(geo_value) %>%
    summarize(value = mean(value, na.rm=TRUE))
...
for (i in inds_county[-length(inds_county)]) {
  ...
  main = paste("jhu-deaths-ahead", "vs", sources[i])
  cor_plot(x, y, county_pop, main=main)
}
