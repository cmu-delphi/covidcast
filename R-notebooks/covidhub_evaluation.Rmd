...
for (i in 1:length(forecasters)) {
  ...
  predictions_cards_list[[i]] = tryCatch({
    get_covidhub_predictions(forecasters[i], 
                             new_dates[[i]]) %>% 
                    filter(ahead < 5) %>% 
                    filter(str_detect(geo_value, "[a-z]{2}")) %>%
                    filter(signal == response_sig)
  },
...
if(exists("score_cards")){
  scored_forecasters = (score_cards %>% distinct(forecaster))$forecaster
} else {
  scored_forecasters = NULL
}
...
theta = pairwise_tournament(score_cards, var = "wis", aggr = 
...
```{r, fig.width = 10, fig.height = 10}
theta = pairwise_tournament(score_cards, var = "wis", aggr = Median)
