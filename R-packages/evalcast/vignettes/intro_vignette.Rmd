...
The `evalcast` package provides the infrastructure for developing and evaluating probabilistic forecasters that are based on data obtained with the [covidcast](https://cmu-delphi.github.io/covidcast/covidcastR/) R package.  A unique feature of the `covidcast` API is that it allows one to get the data that would have been available *as of* a certain date.  This accounts for a difficult property of working with certain COVID-19 data sources, which is that they are backfilled (in other words, the value of some variable on a certain day is updated as time passes).  Not properly accounting for backfill can lead to overly optimistic retrospective evaluations of forecasts and therefore poorly trained forecasters.  The `evalcast` package is designed so that forecasters avoid these pitfalls.
...
```{r, warning=FALSE}
library(evalcast)
library(covidcast) # needed because covidcast make file does not run when only evalcast is libraried

forecast_dates <- get_forecast_dates("CMU-TimeSeries")[1] # Reducing to 1 for testing

#Seems very inefficient, retrieves from 2020-06-15 to each forecast date, so does ~4x the work
predictions_cards <- get_predictions(baseline_forecaster,
                                     ...
                                     ahead = 3,
                                     geo_type = "state")
predictions_cards_yyg <- get_covidhub_predictions("YYG-ParamSearch", forecast_dates)
predictions_cards_cmu <- get_covidhub_predictions("CMU-TimeSeries", forecast_dates)
```

This creates a list of `r length(predictions_cards)` predictions cards, one for each (forecast date, ahead) pairs.  Let's look at a predictions card:

```{r}
predictions_cards_cmu[[1]]
```

Forecasts are probabilistic, meaning that rather than providing a single predicted value, a distribution of values is provided. For example, the baseline forecaster predicts the following distribution in California on ...
```{r}
pc <- predictions_cards_cmu[[1]]
pc[pc$location == "06", ]$forecast_distribution[[1]]
```

### Look at some trajectory plots

Not yet implemented:

```{r, eval=FALSE}
plot_trajectories(predictions_cards)
```
...
scorecards_cmu <- evaluate_predictions(pcards_cmu_death3, 
                                   ...
                                   backfill_buffer = 10)
```
Let's look at the scorecard for `ahead = 4`, i.e., for 4-epiweek ahead forecasts.
```{r}
scorecards[[1]]
```
...
plot_coverage(list(scorecards_yyg[[1]],
                  scorecards_cmu[[1]]))
# plot_width(scorecards_cmu[[1]]) # need to fix plot_width bug (see issue)
```

### Tables of these performance measures

Not yet implemented:

```{r, eval=FALSE}
tabulate_by_location(list(scorecards[[1]],scorecards_othermethod[[1]]))
tabulate_by_time(list(scorecards[[1]],scorecards_othermethod[[1]]))
tabulate(list(scorecards[[1]],scorecards_othermethod[[1]]))
```
