...
Due to inevitable latency in the signals, if we look at training sets defined by
taking all data `n` days before the forecast date, then we don't actually have 
`n` time points worth of data available to us (we're always fetching data "as 
of" the forecast date). As here `n = max(ahead) = 28`, the baseline forecaster 
can't actually produce 28-days-ahead forecasts with the most recent `n` time 
points worth of data, since it would need to calculate differences of signal 
values spaced 28 days apart. This is why we've included an extra 4 days in the 
data we make available to the baseline forecaster (nb. the extra minus 4 in the 
definition of `start_day_baseline()`).

The effect of latency on the quantile forecaster is more complicated. TODO blah

TODO blah get predictions from forecasters. covidcast is 0.4.0 beta release and
evalcast is on evalcast-killcards branch

```{r, eval = FALSE, code = readLines("quantgen-forecast.R")}
```

TODO blah load the data and take a peak at the `results` and `evals` tibbles
...
TODO blah now produce some convenient functions for analysis and plotting
