...
completely unrelated to this one. The Delphi research group's `modeltools` 
package can be installed from Delphi's
...
package (a `modeltools` dependency) adds a couple minutes to the installation. 
If you prefer a quicker installation, you can install `modeltools` without
vignettes (the default setting):
...
the first containing the raw daily case rates and the second containing the 
smoothed case rates. The 
...
As shown in the summary output above, each of the `covidcast_signal` data 
...
signals[[1]] %>% table_head(title = "Daily COVID-19 cases per 100,000 (raw reported)")
signals[[2]] %>% table_head(title = "Daily COVID-19 cases per 100,000 (7-day trailing average)")
```

The `table_head` function used above is a custom function for displaying nice 
table heads in HTML. See the [`vignettes/modeltools.Rmd`](modeltools.Rmd) 
...
slide_by_geo(signals[[1]], slide_fun = ~ Mean(.x$value), n = 7) %>%
  table_head(title = "Daily COVID-19 cases per 100,000 (7-day trailing average)",
             footnote = "Trailing averages computed by sliding a formula")
```
<br />
...
The code chunk below reproduces
...
slide_by_geo(signals[[1]], slide_fun = function(x, ...) Mean(x$value), n = 7, col_name = "7dav") %>%
  table_head(title = "Daily COVID-19 cases per 100,000 (7-day trailing average)",
             footnote = "Trailing averages computed by sliding a function")
```
<br />
