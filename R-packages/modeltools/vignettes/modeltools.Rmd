...
page](https://github.com/cmu-delphi) and can be installed using the commands
below from the [`devtools`](https://cran.r-project.org/package=devtools) 
package.
...
included in the downloaded package. The sans vignette package
...
If you wish to include the vignettes, then use the following installation 
command instead:
...
this case, the passed function must have the following argument structure: 

>`x`: a data frame with the same column names as the data frame passed to
`slide_by_geo()`; followed by any number of named additional arguments; and 
ending with `...`, to allow for general additional arguments. 
