...
predictions_cards_ens <- get_covidhub_predictions(
  "COVIDhub-ensemble", forecast_dates_dec, ahead = 3, 
...
predictions_cards_cmu %>% 
  filter(geo_value == michigan_abr, forecast_date == forecast_dates_july[1]) %>%
...
predictions_cards_ens %>%
  filter(geo_value == california_abr, 
         forecast_date == forecast_dates_july[1]) %>%
...
```{r}
plot_calibration(predictions, geo_type = "state", type = "wedgeplot")
```
...
```{r}
plot_calibration(predictions, geo_type = "state", type = "traditional")
```
...
```{r}
plot_coverage(predictions, geo_type = "state")
```

Or the width of the intervals:

```{r}
plot_width(predictions)
```
