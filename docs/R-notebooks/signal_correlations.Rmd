...
sources = c("doctor-visits", "fb-survey", "fb-survey", "indicator-combination")
...
# Fetch JHU confirmed case incidence proportion (smoothed with 7-day trailing 
# average)
...
signals = c("smoothed_adj_cli", "smoothed_cli", "smoothed_hh_cmnty_cli", 
            "smoothed_search", "nmf_day_doc_fbc_fbs_ght")
names = c("Doctor visits", "Facebook CLI", "Facebook CLI-in-community", 
          "Google trends", "Combo indicator")
...
```{r, message = FALSE, warning = FALSE, fig.width = 7, ig.height = 6}
df_cor = vector("list", length(signals))
for (i in 1:length(signals)) {
  df_cor[[i]] = covidcast_cor(df_signals[[i]] %>% 
                                filter(geo_value %in% geo_values), 
