...
if (exists("predictions_cards")){
  ...
  for (i in 1:length(predictions_cards)){
    ...
    pred_dates = lapply(forecaster_predictions_cards, FUN = function(x) attributes(x)$forecast_date)
    seen_dates[[i]] = unique(do.call("c", pred_dates))
    name_of_forecaster = attributes(forecaster_predictions_cards[[1]])$name_of_forecaster
...
for (i in 1:length(forecasters)) {
  ...
  dates_comp = given_dates[(given_dates %in% forecast_dates_cmu | 
                            ((given_dates + 1) %in% forecast_dates_cmu) &
                            wday(given_dates) == 1)]
  extra_dates = dates_comp[(dates_comp + 1) %in% dates_comp]
  dates_comp = as_date(setdiff(dates_comp, extra_dates))
  if(forecasters[[i]] %in% names(seen_dates)){
    forecaster_idx = which(names(seen_dates) == forecasters[[i]])
    dates_comp = dates_comp[!(dates_comp %in% seen_dates[[forecaster_idx]])]
  }
  new_dates[[i]] = dates_comp
}
...
```{r}
scored_forecasters = unlist(lapply(score_cards, FUN = function(card) attributes(card[[1]])$name_of_forecaster))

ahead = 1:4
last_date = as.Date("2020-10-19")
...
for (i in 1:length(predictions_cards)) {
  ...
  given_dates_trimmed = unique(given_dates[given_dates <= last_date])
...
for (i in 1:length(forecasters)){
  ...
  if (name_of_forecaster %in% scored_forecasters){
    forecaster_idx = which(scored_forecasters == name_of_forecaster)
    score_cards_comb[[i]] = c(score_cards[[forecaster_idx]], score_cards_new[[i]])
  } else {
