...
Filter to forecasters that have significant overlap in predictions with CMU-TimeSeries for the last 4 weeks.This is used primarily to ensure we don't include inactive forecasters in the pairwise tournament rankings and that there is enough overlap for pairwise scores to be meaningful.
...
```{r}
highlight_forecasters = c("CMU-TimeSeries",
                          "COVIDhub-baseline",
                          "COVIDhub-ensemble")
...
To give a fuller view of the coverage of `CMU-TimeSeries` forecasts, we create
a wedge plot per ahead:

```{r, fig.width = 10, fig.height = 10}
type_chars = nchar(score_cards$geo_value[1])
cmu_preds = predictions %>% filter(forecaster == "CMU-TimeSeries", 
                                   nchar(geo_value) == type_chars)

# plot_calibration requires a forecast_date column
cmu_preds_w_cov = left_join(cmu_preds, score_cards, by = c("ahead", "geo_value", "forecaster", "target_end_date")) %>% 
  select(ahead, geo_value, quantile, value, forecaster, target_end_date, forecast_date.x, actual)
colnames(cmu_preds_w_cov)[colnames(cmu_preds_w_cov) == "forecast_date.x"] = "forecast_date"

plot_calibration(cmu_preds_w_cov, grp_vars = c("ahead"), avg_vars = c("geo_value", "forecast_date"), type = "wedgeplot")
```

Also, a coverage plot by ahead and forecast_date:
```{r, fig.width = 10, fig.height = 10}
plot_calibration(cmu_preds_w_cov, grp_vars = c("ahead", "forecast_date"), avg_vars = c("geo_value"), type = "wedgeplot")
```
