...
case_rates <- estimate_deriv(case_rates, method = "smooth-spline", n = 28,
                             new_col = "deriv_ss1", params = list(df = 8))

case_rates <- estimate_deriv(case_rates, method = "smooth-spline", n = 28,
                             new_col = "deriv_ss2", params = list(cv = TRUE))

case_rates %>% 
  arrange(geo_value) %>% 
  filter(time_value <= "2020-06-14") %>%
  select(geo_value, time_value, value, deriv, deriv_ss1, deriv_ss2) 
...
p1 = ggplot(case_rates_fl, aes(x = time_value, y = value)) +
  geom_line() + 
  geom_point(data = case_rates_fl %>% filter(deriv_ss1 >= threshold),
             aes(x = time_value, y = value), color = "red") + 
  geom_point(data = case_rates_fl %>% filter(deriv_ss2 >= threshold),
             aes(x = time_value, y = value), color = "blue", shape = 21) +
  labs(x = "Date", y = "Cases per 100,00 people") 
p2 = ggplot(case_rates_fl, aes(x = time_value)) +
...
## Estimating derivatives via trend filtering spline

TODO, blah try trend filtering two ways, plot them. As it takes a while (here 
using solution path algorithm from `genlasso` package), just compute for Florida

```{r, fig.width = 8, fig.height = 4}
case_rates_fl <- estimate_deriv(case_rates_fl, method = "trend-filter", n = 28,
                                new_col = "deriv_tf1", params = list(df = 8))

case_rates_fl <- estimate_deriv(case_rates_fl, method = "trend-filter", n = 28,
                                new_col = "deriv_tf2", params = list(cv = TRUE))

threshold = 0.25

p1 = ggplot(case_rates_fl, aes(x = time_value, y = value)) +
  geom_line() + 
  geom_point(data = case_rates_fl %>% filter(deriv_tf1 >= threshold),
             aes(x = time_value, y = value), color = "red") + 
  geom_point(data = case_rates_fl %>% filter(deriv_tf2 >= threshold),
             aes(x = time_value, y = value), color = "blue", shape = 21) +
  labs(x = "Date", y = "Cases per 100,00 people") 
p2 = ggplot(case_rates_fl, aes(x = time_value)) +
