...
threats to public health, with an emphasis on short-term forecasting. It is 
...
An <tt>R</tt> package named "`modeltools`" does exist on CRAN, but it is entirely 
unrelated to this one. Our `modeltools` package can be installed from the 
Delphi research group's 
[GitHub page](https://github.com/cmu-delphi/) using the
[`devtools`](https://cran.r-project.org/package=devtools) package, as shown
below. The source code for the package can be found
[here](https://github.com/cmu-delphi/covidcast/tree/main/R-packages/modeltools).


### Installing with vignettes

If you wish to install the `modeltools` package with the tutorial vignettes 
included (encouraged), then use the command below.
...
package (a dependency of the `modeltools` package) adds a couple minutes to the 
package installation. If you wish to avoid this, you can install the 
`modeltools` package without vignettes with the following command:
...
For this **_Getting started_** vignette, we will fetch U.S. COVID-19 case rates --- 
both raw and smoothed with 7-day trailing averages --- from
[**Delphi's Epidata API**](https://cmu-delphi.github.io/delphi-epidata/api/covidcast_signals.html). 
These particular COVID-19 data signals are reported by
[**USAFacts**](https://usafacts.org/)
and then cloned into Delphi's API. Here, we will download these COVID-19 case 
rates at the state level over the time window June 1, 2020 to November 15, 2020 
for four states --- CA, FL, NY, and TX.
...
function returns the specified signals as 
...
case rates after smoothing with a 7-day trailing average. The 
...
Taking a peek at the first 5 rows of each of the two `covidcast_signal` data 
frames in the list:
...
specify a formula via the `slide_fun` argument of `slide_by_geo()`, as follows:
...
slide_by_geo(signals[[1]], slide_fun = ~ Mean(.x$value), n = 7) %>%
  ...
  head(10) %>%
  kable(format = "html", digits = 3, align = "c",
        caption = paste("U.S. COVID-19 case rates (7-day trailing average", 
                        "computed by sliding a formula)"))
```
...
column appended containing the results of the sliding operation specified by
`slide_fun`. By default, this column is named `"slide_value"`, but alternative
...
As a simple sanity check, below we compare the 7-day trailing averages computed 
via `slide_by_geo()` to the pre-computed 7-day trailing averages stored 
in Delphi's Epidata API to ensure that they match (to machine precision). 
More specifically, the code chunk below computes the maximum absolute difference
between the two sets of 7-day trailing averages over all state-date pairs:
...
The trivially small output of this sanity check confirms that `slide_by_geo()` 
is functioning as desired. <br/><br/>

**_Note_: This sanity check would fail** without filtering the dates to 
`>= "2020-06-07"` because we are only providing `slide_by_geo()` with daily 
COVID-19 case rates beginning on June 1, 2020, so the "7-day trailing average" 
computed by `slide_by_geo()` does not become a true *7-day average* until 
June 7, 2020. When a trailing window of `n` days is not available, 
`slide_by_geo()` just applies the function or formula to whatever data *is* 
available. So the "7-day trailing average" computed by `slide_by_geo()` for the
dates June `n`th where `n = 1,...,6`, is actually just an `n`-day trailing 
average of the daily COVID-19 case rates. On the other hand, the pre-computed 
7-day trailing averages stored in Delphi's Epidata API are true 7-day averages
for all dates June 1, 2020, $\dots$, November 15, 2020.
...
The code chunk below recreates 
[Example 1][Example 1: Computing a 7-day trailing average by sliding a formula over the raw data], 
