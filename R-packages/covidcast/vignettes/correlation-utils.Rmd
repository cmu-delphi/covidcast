...
```{r, include = FALSE}
library(httptest)
start_vignette("correlation-utils")
```
...
across all states in the us over roughly five months.
...
# Compute correlation per time, over all states
df_cor1 <- covidcast_cor(inum, dnum, by = "time_value")
...
ggplot(df_cor1, aes(x = time_value, y = value)) + geom_line() + 
  labs(title = "Correlation between cases and deaths",
       subtitle = "Per day, over states",
       x = "Date", y = "Correlation")
```

(The drop in mid-June is probably related to there being a lull in reported
cases in this time period, and hence there being less variation between
states---less signal, more noise. In July and August, reported case counts began
to rise.)
...
df_cor2 <- covidcast_cor(inum, dnum, by = "time_value", dt_x = -10)
...
```{r, warning = FALSE}
# Compute correlation per state, over all times
df_cor1 <- covidcast_cor(inum, dnum, by = "geo_value")
df_cor2 <- covidcast_cor(inum, dnum, by = "geo_value", dt_x = -10)
...
ggplot(df_cor, aes(value)) + 
  ...
  labs(title = "Correlation between cases and deaths",
       subtitle = "Computed separately for each state, over all times",
       x = "Date", y = "Density") +
  theme(legend.position = "bottom")
```
...
attributes(df_cor2)$metadata$geo_type <- "state"
...
for (i in 1:length(dt_vec)) {
  df_list[[i]] <- covidcast_cor(inum, dnum, dt_x = dt_vec[i],
                               by = "geo_value")
...
incidence numbers back 10 days in time.
