...
[`evalcast`](https://cmu-delphi.github.io/covidcast/evalcastR/) packages, which
are also developed and maintained by the [Delphi Group](https://delphi.cmu.edu) 
...
This package is available on the [CMU Delphi Group GitHub 
page](https://github.com/cmu-delphi) and can be installed using the 
[`devtools`](https://cran.r-project.org/package=devtools) package, as follows:

```{r, eval = FALSE}
devtools::install_github(repo = "cmu-delphi/covidcast", ref = "main",
                         subdir = "R-packages/modeltools")
```

Building the package's vignettes, such as this *Getting started guide*, takes a 
significant amount of time. So, by default, the vignettes are not included 
with the downloaded package. If you wish to include the vignettes, then use 
the following installation command instead:
...
For this *Getting started* vignette, we'll fetch COVID-19 case rates, both raw 
and smoothed via 7-day trailing averages, from the [Delphi 
API](https://cmu-delphi.github.io/delphi-epidata/api/covidcast_signals.html). 
These particular signals are provided by the [*USAFacts*](https://cmu-delphi.github.io/delphi-epidata/api/covidcast-signals/usa-facts.html)
data source. Here, we'll download the signals at the state level for four 
states, and over the time window of June 1, 2020 to November 15, 2020.
...
package. In `modeltools`, to *slide* means, for each `geo_value`, to apply the 
function or formula over a trailing window of `n` days of data. Many other 
functions in the package, such as `pct_change()` and `estimate_deriv()`, use 
...
original `slide_by_geo()` call. For example: 
...
Note that this check would *fail* before June 7 because the API has access to 
data before June 1, 2020, and can therefore return the trailing 7-day average
for all days in the time window June 1, 2020 to November 15, 2020. While,
on the other hand, since we are only providing `slide_by_geo()` with daily 
COVID-19 case rates beginning on June 1, the trailing 7-day average computed 
by `slide_by_geo()` is not actually an average of 7 days of data until June 7,
2020. When a trailing window of `n` days isn't available, `slide_by_geo()` just 
applies the function or formula to whatever data is available.
...
a data frame with the same column names as the data frame passed to
`slide_by_geo()`; followed by any number of named additional arguments; and 
ending with `...`, to allow for general additional arguments. Recreating the 
last example of a 7-day trailing average: 
