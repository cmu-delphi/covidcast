...
While `evalcast` provides a number of tools to visually assess a forecaster (including comparisons with other forecasters), because `score_cards` are just long data frames, many plots can be achieved using [{ggplot2}](http://ggplot2.tidyverse.org).
...
Of course, if we had multiple aheads we could facet by those as well. Or we could assume that aheads and forecast dates are comparable and include them all in the boxplots, if the assumption is reasonable.

We can also examine forecaster calibration.

```{r}
plot_calibration(scorecards, type = "wedgeplot")
```

By default, the proportion is the number of `geo_values` that fall above/below the quantile at each forecast date and ahead. But this can be changed if desired.

```{r}
plot_calibration(scorecards, 
                 grp_vars = "forecaster", 
                 avg_vars = c("geo_value", "forecast_date"))
```

One can also examine a more traditional calibration plot

```{r}
plot_calibration(scorecards, type = "traditional")
```
