---
title: 4. Produce and evaluate quantile forecasts of a COVIDcast signal
description: Produce and evaluate quantile forecasts of a COVIDcast signal, 
  using on `quantgen` and `evalcast`.
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{4. Produce and evaluate quantile forecasts of a COVIDcast signal}
...
Note that I'm using `covidcast` from the `r/0.4.0-release` branch, and 
`evalcast` from the `evalcast-killcards` branch.
...
Load the data and take a peek at the `evals` tibble
...
```{r, message = FALSE, warning = FALSE, fig.align = "center", fig.width = 7, fig.height = 5, fig.retina = 2}
...
of operations, and median for robustness)
