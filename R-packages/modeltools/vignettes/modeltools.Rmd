...
[Delphi Group](https://delphi.cmu.edu) at Carnegie Mellon University. 
...
This package is available on the [Delphi Group GitHub 
...
can take up to a couple minutes.
...
case rates smoothed by a 7-day trailing average. The `summary()` function can be
called on each of these `covidcast_signal` data frames as follows:
...
signals[[1]] %>%
  ...
  head(5) %>%
  kable(format = "html", digits = 3, caption = "U.S. COVID-19 case rates (raw)")

signals[[2]] %>%
  select(data_source, signal, geo_value, time_value, value) %>% 
  head(5) %>%
  kable("html", digits = 3, caption = "U.S. COVID-19 case rates (7-day trailing average)")
```
...
that they match (within machine precision). More specifically, the code chunk
...
`>= "2020-06-07"` because the API has access to data before June 1, 2020, and 
can therefore return the trailing 7-day average
