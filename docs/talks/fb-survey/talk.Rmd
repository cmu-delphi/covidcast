...
author: Katie Mazaitis\* and Ryan Tibshirani^†^ <br> \*^†^Machine Learning and ^†^Statistics
  <br> Carnegie Mellon University <br> ^†^Amazon Scholar, AWS AI Labs
date: "<br> ![](delphi.png) ![](cmu.png) <br><br> September 1, 2020"
footer: 'Get the slides at: cmu-delphi.github.io/covidcast/talks/intro-api/'
output: 
  slidy_presentation:
    theme: cerulean
    highlight: tango
...
Next talks: Facebook surveys, medical claims data, ...
...
deaths = covidcast_signal(data_source = "jhu-csse", 
                          ...
                          start_day = "2020-03-01", geo_type = "state", 
                          geo_values = "pa")

plot(deaths, plot_type = "line", 
     title = "COVID-19 deaths in PA (7-day trailing avg)")
```
...
plot(cases, choro_col = c("#D3D3D3", "#FFC0CB"), 
     choro_params = list(breaks = c(0, 2000)),
...
dv = covidcast_signal(data_source = "doctor-visits", 
                      ...
                      start_day = "2020-03-01", geo_type = "msa", 
...
The COVIDcast API is based on HTTP GET queries and returns data in JSON form. The base URL is https://delphi.cmu.edu/epidata/api.php?source=covidcast
...
Estimated % COVID-like illness on April 6, 2020 from the Facebook survey, in Allegheny County: https://delphi.cmu.edu/epidata/api.php?source=covidcast&data_source=fb-survey&signal=raw_cli&time_type=day&geo_type=county&time_values=20200406&geo_value=42003 

```{r}
library(jsonlite)
res = readLines("https://delphi.cmu.edu/epidata/api.php?source=covidcast&data_source=fb-survey&signal=raw_cli&time_type=day&geo_type=county&time_values=20200406&geo_value=42003")
prettify(res)
```
...
| :-- | :-- | :-- |
| `as_of` | to get data as if we queried the API on a particular date | `20200406` |
| `issues` | to get data published at a particular date or date range | `20200406` or `20200406-20200410` |
| `lag` | to get data published a certain number of time units after events occured | `1` or `3` |
...
- Medical claims data can take days to be submitted and/or processed
...
Still highly under development ... much more to come. Current vignettes:
...
(You can contribute via a pull request on our public GitHub repo!)
...
# Example: Correlations With Survey Signals

```{r, fig.width=7, fig.height=5}
# Fetch Facebook % CLI signal, % CLI-in-community signal and JHU confirmed case
# incidence proportions
start_day = "2020-04-15"
sympt1 = covidcast_signal("fb-survey", "smoothed_cli", start_day)
sympt2 = covidcast_signal("fb-survey", "smoothed_hh_cmnty_cli", start_day)
cases = covidcast_signal("jhu-csse", "confirmed_7dav_incidence_prop", start_day)
...
# Example: Correlations With Survey Signals (Cont.)
