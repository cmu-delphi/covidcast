---
title: 2. Estimating the derivatives of a covidcast signal
description: Estimating the derivatives of a covidcast signal, using various 
  methods.
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{2. Estimating the derivatives of a covidcast signal}
...
we demonstrate this derivative estimation functionality on smoothed versions of 
the state-level COVID-19 case rates reported by the
[*USAFacts*](https://cmu-delphi.github.io/delphi-epidata/api/covidcast-signals/usa-facts.html)
data source from June 1, 2020 to November 15, 2020. The code chunk below fetches
these data from the 
...
`method = "lin"`, which uses the slope from a simple linear regression. Here 
we also use the default trailing number of days `n = 14`.
...
with the signal itself. The <span style="color: red;">red</span> dots mark time points at which the derivative 
...
[`vignettes/estimate-deriv.Rmd`](estimate-deriv.Rmd) 
file used to build this page.

```{r, message = FALSE, warning = FALSE, fig.align = "center", fig.width = 8, fig.height = 4, fig.retina = 2, dpi = 85}
library(ggplot2)
library(gridExtra)
...
```{r, warning = FALSE, fig.align = "center", fig.width = 8, fig.height = 4, fig.retina = 2, dpi = 85}
