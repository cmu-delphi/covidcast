---
title: 1. Compute the percentage change of a `covidcast` signal over time
description: Compute the percentage change of `covidcast` signal values over time.
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{1. Compute percentage change of a `covidcast` signal over time}
...
A simple way of assessing the growth or decay of a `covidcast` signal is to look 
at the percentage change in its cumulative value between two adjacent time 
...
## Post-hoc smoothing percentage change time series
...
percentage change time series (shown in red). As expected, a direct calculation
of the percentage change in COVID-19 case rates at the daily level yields a 
highly volatile time series that is almost surely not representative of real 
variations in the spread of COVID-19 in Florida. The daily percentage change
post-processed with a 7-day trailing average (shown in blue) is likely a better
indicator.

```{r, message = FALSE, warning = FALSE, fig.width = 8, fig.height = 4.5}
p1 <- ggplot(case_rates %>% filter(geo_value == state), aes(x = time_value, y = value)) +
  geom_line() + 
  labs(title = "Florida COVID-19 case rates (smoothed)", 
       x = "Date", y = "Cases per 100,000 people") + 
