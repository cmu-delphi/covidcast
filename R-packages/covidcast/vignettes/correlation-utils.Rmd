...
TODO: explain that some outlier cause this bizarre dip in correlation? 

We might also be interested in how cases now correlate with deaths in the
*future*. Using the `dt_x` parameter, we can shift the signal by 10 days forward
in time, before calculating correlations:
...
We can see that, for the most part, shifting the cases time series forward by 10
days improves correlations.
...
The "big dip" is gone, and we can again see that the shift helps correlations.
...
both for observations at the same time and for a 10-day shift:
...
# Loop over values for the time shift, and compute correlations per county
dt_vec = 0:15
df_list = vector("list", length(dt_vec))
for (i in 1:length(dt_vec)) {
  df_list[[i]] = covidcast_cor(df_inum_act, df_dnum_act, dt_x = dt_vec[i],
                               by = "geo_value")
...
correlations come from slicing by location) is maximized when we shift the case
incidence numbers 8 days forward in time.